...
void print_usage(const char* program) {
    ...
    std::cerr << "  --M <n>                    max graph degree parameter (default: 16)\n";
    std::cerr << "  --ef-construction <n>      construction/search candidate list size (default: 32)\n";
}
...
bool parse_args(int argc, char** argv, Config& cfg) {
    ...
    for (int i = 4; i < argc; i++) {
        ...
        if (arg == "--dynamic") {
            ...
            cfg.ef_construction = std::stoi(argv[++i]);
        } else {
...
int run_dynamic_benchmark(const std::vector<std::vector<float>>& train, Metric metric, const Config& cfg) {
    ...
    HNSW index(base, cfg.M, cfg.ef_construction, metric);
    ...
    std::cout << "Dataset: " << cfg.dataset_name
              ...
              << ", M=" << cfg.M
              << ", ef_construction=" << cfg.ef_construction << std::endl;
    ...
    for (int i = 0; i < cfg.op_count; i++) {
        int id = index.insert(train[cfg.base_count + i]);
        if (id < 0) return 1;
...
int main(int argc, char** argv) {
    ...
    HNSW index(train, cfg.M, cfg.ef_construction, metric);
    auto start = std::chrono::high_resolution_clock::now();
    index.build();
    auto end = std::chrono::high_resolution_clock::now();
    ...
    std::vector<int> ef_search_list = {1, 3, 4, 5, 6, 7, 8, 9, 10, 15, 20, 30, 50, 100, 150, 200, 300, 500};
    int k = 1;
    ...
    for (int ef_search : ef_search_list) {
        std::cout << "\nTesting with ef_search=" << ef_search << std::endl;

        // Warm up
        for (int i = 0; i < std::min(10, (int)test.size()); i++) {
            index.search(test[i], k, ef_search);
        }
        ...
        for (const auto& query : test) {
            auto [result, stats] = index.search(query, k, ef_search);
